...
/**
 * FIXED: Delete employee with user email for archive tracking
 * @param {number} id - Employee ID to delete
 * @param {string} deletedByEmail - Email of user performing deletion
 */
export const deleteEmployee = async (id, deletedByEmail) => {
  const response = await api.delete(`/employees/${id}`, {
    data: { deleted_by: deletedByEmail }
  });
  return response.data;
};
