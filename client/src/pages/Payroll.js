...
//After the payroll history upto 12 months..
...
function Payroll() {
  ...
  const loadPayrollHistory = async () => {
    try {
      ...
      // Load last 12 months of payroll
      for (let i = 0; i < 12; i++) {
        ...
        try {
          ...
          if (response.data && response.data.length > 0) {
            const totalPayroll = response.data.reduce((sum, p) => sum + parseFloat(p.net_salary || 0), 0);
