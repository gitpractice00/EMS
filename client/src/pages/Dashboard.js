...
//Ater the password change features
import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { getAllEmployees, logout, getCurrentUser, deleteEmployee } from '../services/api';
...
function Dashboard() {
  ...
  // FIXED: Update user state when profile is updated
  const handleProfileUpdate = (updatedUser) => {
    setUser(updatedUser);
  };

  // FIXED: Open delete confirmation modal
  const handleDelete = (id, name) => {
    setEmployeeToDelete({ id, name });
    setIsDeleteModalOpen(true);
  };

  // FIXED: Confirm delete with password verification
  const handleConfirmDelete = async (password) => {
    if (!employeeToDelete) return;

    try {
      await deleteEmployee(employeeToDelete.id, password);
      alert(`${employeeToDelete.name} deleted successfully!`);
  ...
  return (
    <div className="dashboard">
      ...
      <header className="top-header">
        ...
        <div className="user-section">
          {/* FIXED: Made user-info clickable */}
