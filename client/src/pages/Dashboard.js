...
//   // FIXED: Delete without password - pass null or empty string to API
...
//     try {
//       // Call deleteEmployee without password (pass null)
//       await deleteEmployee(id, null);
      
//       // Update the local state immediately to remove the employee from UI
...
//       // Reload employees to ensure UI is in sync with backend
//       loadEmployees();
//     }
//   };
...
function Dashboard() {
  ...
  // FIXED: Pass user email to deleteEmployee for archiving
  const handleDelete = async (id, name) => {
    const confirmed = window.confirm(`Are you sure you want to delete ${name}?\n\nThis action cannot be undone.`);
    ...
    try {
      // FIXED: Pass current user's email for archive tracking
      await deleteEmployee(id, user?.email);
