...
function AddEmployee() {
  ...
  const [existingPhones, setExistingPhones] = useState([]); // FIXED: Added phone state

  const loadExistingEmails = async () => {
    try {
      const response = await getAllEmployees();
      const emails = response.data.map(emp => emp.email.toLowerCase());
      const phones = response.data.map(emp => emp.phone).filter(Boolean); // FIXED: Load phones
      setExistingEmails(emails);
      setExistingPhones(phones); // FIXED: Set phones
    } catch (err) {
      console.error('Failed to load existing employees:', err);
    }
  };
  ...
  const validateForm = () => {
    ...
    // FIXED: Check if phone is unique
